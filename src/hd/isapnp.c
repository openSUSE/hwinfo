    ...
    for(j = 0; j < c->log_devs; j++) {
      ...
      if(
        ...
        if((r = get_isapnp_res(c, 0, RES_ANSI_NAME))) {
          ...
          add_device_name(hd->vend, hd->dev, 0, s);
          free_mem(s);
        }
      }
      ...
      if((r = get_isapnp_res(c, j + 1, RES_LOG_DEV_ID))) {
        ...
        if((u = sub_device_class(hd->vend, hd->dev, hd->sub_vend, hd->sub_dev))) {
          hd->base_class = u >> 8;
          hd->sub_class = u & 0xff;
        }
        ...
        if(
          ...
          if((r = get_isapnp_res(c, j + 1, RES_ANSI_NAME))) {
            s = canon_str(r->data, r->len);
            add_sub_device_name(hd->vend, hd->dev, hd->sub_vend, hd->sub_dev, 0, s);
            free_mem(s);
          }
        }
      }
