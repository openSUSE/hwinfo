...
int main( int argc, char **argv, char **envp )
{
	...
	envstr = getenv("HWSCAN_DEBUG");
	if (envstr && !strcmp(envstr,"1")) {
		debug = 1;
		envstr = NULL;
	}

	envstr = getenv("HWSCAN_TIMEOUT");
	if (envstr) {
		timeout = strtoul(envstr,&endp, 10);
		if (envstr == endp) {
			timeout = TIMEOUT;
		}
		if (debug)
			printf("hwscand: timeout %lu seconds\n", timeout);
		envstr = NULL;
	}
